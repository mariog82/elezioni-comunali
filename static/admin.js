...
async function loadDashboard(){
  try{
    ...
    d.sections.forEach(s=>{
      const calculated=(s.total_lists||0)+(s.blank_ballots||0)+(s.null_ballots||0)+(s.contested_ballots||0);
...
function renderBallotSummary(d){
  ...
  const calculated=validLists+(b.blank_ballots||0)+(b.null_ballots||0)+(b.contested_ballots||0);
  ...
  document.getElementById("ballotSummary").innerHTML=`
    ...
    <b>Schede bianche:</b> ${b.blank_ballots||0} &nbsp; <b>Nulle:</b> ${b.null_ballots||0} &nbsp; <b>Contestate:</b> ${b.contested_ballots||0}<br>
    <b>Totale controllo:</b> ${calculated}<br>
