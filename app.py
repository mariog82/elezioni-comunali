...
def save_report():
    ...
    expected_voters = valid_votes + blank_ballots + null_ballots + contested_ballots
    if voters != expected_voters:
        return jsonify({"ok": False, "error": f"Controllo votanti non superato: votanti={voters}, totale calcolato={expected_voters}. I votanti devono essere uguali a voti validi + bianche + nulle + contestate."}), 400
    now = datetime.now().isoformat(timespec="seconds")
    conn = db()
    cur = conn.cursor()
...
def dashboard():
    ...
    sections = conn.execute("""
        SELECT r.section, u.name AS representative, r.updated_at,
          r.voters, r.blank_ballots, r.null_ballots, r.contested_ballots,
...
def export_csv():
    ...
    """).fetchall()
    conn.close()
    lines = ["Sezione;Rappresentante;Votanti;Bianche;Nulle;Contestate;Aggiornato;Tipo;Lista;Nome;Voti"]
    for row in rows:
        lines.append(f"{row['section']};{row['representative']};{row['voters']};{row['blank_ballots']};{row['null_ballots']};{row['contested_ballots']};{row['updated_at']};{row['vote_type']};{row['list_name'] or ''};{row['name']};{row['votes']}")
